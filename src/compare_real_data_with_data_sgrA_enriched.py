...
def plot_residuals(df, residuals, label):
    ...
    plt.grid(True)
    plt.tight_layout()
    plot_path = f"validation/plots/{label.lower().replace(' ', '_')}_residualsv2.png"
    os.makedirs("validation/plots", exist_ok=True)
    plt.savefig(plot_path)
    return plot_path

def write_md_summary(source, features, score, plot_path, path="validation/axisw_real_data_comparison_with_sgrA_enriched.md"):
